...
def main():
    ...
    for peek in range(360,830,5):
        if c.TARGET == 'file':
            ...
            wl0, wl1, step = 300, 2000, 10
            peaks = [
                {"anchor": peek,  "fwhm": 100,  "amplitude": 0.7},
            ]
            baseline_segments = [
        ...
        if c.TARGET == 'file':
            ...
            sorted_by_first = sorted(per_sample_results, key=lambda x: np.mean(np.absolute(target_comp[6:44]-x['pred_spectrum'][6:44])))
            # sorted_by_first = sorted(per_sample_results, key=lambda x: np.mean(np.square(target_comp[6:44]-x['pred_spectrum'][6:44])))
