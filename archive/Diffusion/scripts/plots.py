...
def train_data_comp(
    ...
    for number, _ in enumerate(data):
        # print(number)
        ACC, denoise, MAE, MSE, RAT_pred, RAT_tar, stack_pred, stack_tar = \
        data[number]['acc'], data[number]['denoise_steps'], data[number]['mae'], data[number]['mse'], data[number]['RAT_pred_flat'], data[number]['RAT_target_flat'], data[number]['stack_pred_tokens'], data[number]['stack_target_tokens']
        # ---------------------------------------------------------------
        ...
        if RAT_closest.size(0) == 3:
            RAT_closest = RAT_closest.reshape(-1)
        
        SEQ_closest = [idx_to_token[toke] for toke in SEQ_closest.tolist() if toke not in [eos_idx, pad_idx, msk_idx]]
        mae_closest = np.mean(np.absolute((torch.tensor(RAT_tar) - RAT_closest).tolist()))
        RAT_tar_crop = torch.cat([torch.tensor(RAT_tar)[10:81],torch.tensor(RAT_tar)[171*2+10:171*2+81]])
        RAT_closest_crop = torch.cat([RAT_closest[10:81],RAT_closest[171*2+10:171*2+81]])
        mae_closest_crop = np.mean(np.absolute((RAT_tar_crop - RAT_closest_crop).tolist()))
        spectrum_mae = MAE
        
        # MAE_scatter[number] = [number,spectrum_mae,mae_closest]
        MAE_scatter[number] = {'number': number, 'spectrum_mae': spectrum_mae, 'mae_closest': mae_closest, 'mae_closest_crop': mae_closest_crop}
        ...
        if sample_plots and number%int(len(data)/5)==0:
            ...
            # compact helper
            def _strip(seq): return [t for t in seq if t not in ("<PAD>", "<MSK>", "<EOS>")]
            ...
            plt.text(0.24, -0.12,
                     "Target:\n" + "\n".join(f"{m.split('_')[0]}" for m in stack_tar),
                     ha='left', fontsize=10, transform=plt.gca().transAxes,
                     horizontalalignment='left',verticalalignment='top',)
            plt.text(0.46, -0.16,
                     "\n".join(f"{m.split('_')[1]}" for m in stack_tar),
                     ha='right', fontsize=10, transform=plt.gca().transAxes,
                     horizontalalignment='left',verticalalignment='top',)
        
            # → Prediction
            plt.text(0.50, -0.12,
                     "Prediction:\n" + "\n".join(f"{m.split('_')[0]}" for m in stack_pred),
                     ha='left', fontsize=10, transform=plt.gca().transAxes,
                     horizontalalignment='left',verticalalignment='top',)
            plt.text(0.72, -0.16,
                     "\n".join(f"{m.split('_')[1]}" for m in stack_pred),
                     ha='right', fontsize=10, transform=plt.gca().transAxes,
                     horizontalalignment='left',verticalalignment='top',)
            
            # → Trainset
            plt.text(0.76, -0.12,
                     "Closest in Train:\n" + "\n".join(f"{m.split('_')[0]}" for m in SEQ_closest),
                     ha='left', fontsize=10, transform=plt.gca().transAxes,
                     horizontalalignment='left',verticalalignment='top',)
            plt.text(0.98, -0.16,
                     "\n".join(f"{m.split('_')[1]}" for m in SEQ_closest),
                     ha='right', fontsize=10, transform=plt.gca().transAxes,
                     horizontalalignment='left',verticalalignment='top',)
