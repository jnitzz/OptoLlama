...
def Call_RayFlare_with_dict(c, stack_str_list, EOS_TOKEN):
    """
    ...
    for item in stack_str_list:
        if "_" not in item:
            ...
            if item == EOS_TOKEN:
                ...
                continue
        mat_str, thick_str = item.split("_")
        if thick_str[0] == '-':
            thick_str = '0'
    ...
    RAT = tmm_structure.calculate(struc, options, profile=False)
    # Flatten R, A, T
    R = torch.tensor(RAT['R'])
    A = torch.tensor(RAT['A'])
    T = torch.tensor(RAT['T'])
    # out_arr = np.concatenate([R, A, T])
    
    R = torch.nan_to_num(R, nan=0.0, posinf=0.0, neginf=0.0).float().clamp_(0.0, 1.0)
    T = torch.nan_to_num(T, nan=0.0, posinf=0.0, neginf=0.0).float().clamp_(0.0, 1.0)
